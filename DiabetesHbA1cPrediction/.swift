...
struct GlucoseCurveProcessor {
    ...
    static let mealLookbackMinutes: Int = 60
    ...
    private static func thinForCGM(
        ...
        for i in 0..<sorted.count {
            guard spikeAnnotations[i] != nil,
                  let spikeTs = sorted[i].timestamp else { continue }
            for j in 0..<sorted.count {
                guard let ts = sorted[j].timestamp else { continue }
                if abs(ts.timeIntervalSince(spikeTs)) <= spikeMarginSec {
                    nearSpike.insert(j)
                }
            }
        }
