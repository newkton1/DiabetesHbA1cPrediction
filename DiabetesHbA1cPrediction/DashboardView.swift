...
/// Uses SwiftUI Charts framework for visualization with line chart.
/// Color coding based on NGSP% thresholds:
/// - Green: NGSP% ≤6.5 (IFCC ≤47 mmol/mol)
/// - Red: NGSP% >6.5 (IFCC >47 mmol/mol)
private struct GlucoseTrendChartView: View {
    let glucoseReadings: [GlucoseReadingEntity]

    /// NGSP% threshold for color coding (6.5%)
    /// Corresponds to IFCC 47 mmol/mol
    private let ngspThreshold: Double = 6.5
    ...
    /// Convert average glucose (mg/dL) to estimated HbA1c (NGSP%)
    /// Using the Nathan formula: HbA1c = (eAG + 46.7) / 28.7
    private func glucoseToNGSP(_ glucoseMgDL: Double) -> Double {
        return (glucoseMgDL + 46.7) / 28.7
    }

    /// Determine line color based on NGSP% threshold
    /// Green: NGSP% ≤6.5 (IFCC ≤47 mmol/mol)
    /// Red: NGSP% >6.5 (IFCC >47 mmol/mol)
    private func lineColor(for glucoseValue: Double) -> Color {
        let ngsp = glucoseToNGSP(glucoseValue)
        return ngsp <= ngspThreshold ? .green : .red
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            Chart {
                ForEach(dailyAverages, id: \.day) { item in
                    ...
                    .foregroundStyle(lineColor(for: item.average))
    ...
    var body: some View {
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                ...
                StatCard(
                    title: "Exercise (Week)",
                    value: "\(exerciseMinutesWeek) minutes",
                    icon: "figure.walk",
                    iconColor: .blue
                )
            }
